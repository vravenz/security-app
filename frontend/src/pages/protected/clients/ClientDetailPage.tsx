...
const ClientDetailPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (!companyId) return;
            try {
                const clientResponse = await axios.get(`http://localhost:4000/api/clients/company/${companyId}/details/${clientId}`);
                setClient(clientResponse.data);
                const groupsResponse = await axios.get(`http://localhost:4000/api/clients/${clientId}/groups`);
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            const response = await axios.post(`http://localhost:4000/api/clients/${clientId}/groups`, groupData);
