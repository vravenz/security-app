...
const EmployeeDetailPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchEmployeeDetail = async () => {
            try {
                const response = await axios.get(`http://localhost:4000/api/employees/details/${applicantId}`);
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        if (employeeDetail) {
            try {
                const response = await axios.patch(`http://localhost:4000/api/employees/details/${applicantId}`, employeeDetail);
