...
const DeletedEmployeesPage: React.FC = () => {
    ...
    const fetchDeletedEmployees = async () => {
        if (companyId) {
            try {
                const response = await axios.get(`http://localhost:4000/api/${companyId}/deleted-employees`);
    ...
    const restoreEmployee = async (applicantId: number) => {
        try {
            await axios.patch(`http://localhost:4000/api/employees/restore/${applicantId}`);
