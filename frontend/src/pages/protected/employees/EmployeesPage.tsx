...
const EmployeesPage: React.FC = () => {
    ...
    const fetchEmployees = async () => {
        if (companyId) {
            try {
                const response = await axios.get(`http://localhost:4000/api/${companyId}/employees`);
    ...
    const deleteEmployee = async (applicantId: number) => {
        try {
            await axios.delete(`http://localhost:4000/api/employees/details/${applicantId}`);
    ...
    const makeDormant = async (applicantId: number) => {
        try {
            await axios.patch(`http://localhost:4000/api/employees/${applicantId}/dormant`, { dormant: true });
