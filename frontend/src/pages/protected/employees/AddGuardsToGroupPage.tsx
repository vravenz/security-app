...
const AddGuardsToGroupPage: React.FC = () => {
    ...
    const fetchEmployees = async () => {
        try {
            const allEmployeesResponse = await axios.get(`http://localhost:4000/api/${companyId}/employees`);
    ...
    const fetchGroupGuards = async () => {
        try {
            const response = await axios.get(`http://localhost:4000/api/guard-groups/${groupId}/guards`);
    ...
    const handleAddGuards = async () => {
        ...
        try {
            await axios.post('http://localhost:4000/api/guard-groups/add-guards', {
    ...
    const handleRemoveGuard = async (applicantId: number) => {
        try {
            await axios.delete('http://localhost:4000/api/guard-groups/remove-guard', {
    ...
    const extendedEmployees = useMemo(() => {
        return employees.map((employee) => ({
            ...
            employeePhoto: employee.employee_photo ? (
                <img 
                    src={`http://localhost:4000/uploads/employee-photos/${employee.employee_photo}`} 
                    alt="Employee" 
    ...
    const extendedGroupGuards = useMemo(() => {
        return groupGuards.map((guard: Employee) => ({
            ...
            employeePhoto: guard.employee_photo ? (
                <img 
                    src={`http://localhost:4000/uploads/employee-photos/${guard.employee_photo}`} 
                    alt="Employee" 
                    style={{ width: '50px', height: '50px', borderRadius: '50%' }} 
                />
            ) : 'No photo',
