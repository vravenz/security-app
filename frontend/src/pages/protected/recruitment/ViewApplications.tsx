...
const ViewApplications: React.FC = () => {
    ...
    const fetchApplications = async () => {
        if (companyId) {
            try {
                const response = await fetch(`http://localhost:4000/api/applicants/${companyId}`);
                if (response.ok) {
    ...
    const updateStatus = async (applicationId: number, status: string) => {
        const response = await fetch(`http://localhost:4000/api/applicants/update-status/${applicationId}`, {
            method: 'PATCH',
            headers: {
    ...
    const extendedApplications = applications.map(app => ({
        ...
        employeePhoto: app.employeePhoto ? (
            <img 
                src={`http://localhost:4000/uploads/employee-photos/${app.employeePhoto}`} 
                alt="Employee Photo" 
