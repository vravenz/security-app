...
const ShortlistedPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchShortlistedApplications = async () => {
            if (companyId) {
                try {
                    const response = await fetch(`http://localhost:4000/api/applicants/shortlisted/${companyId}`);
                    if (response.ok) {
    ...
    const handleScheduleInterview = async () => {
        if (selectedApplicationId) {
            try {
                const response = await fetch(`http://localhost:4000/api/applicants/interviews/${selectedApplicationId}`, {
                    method: 'POST',
                    headers: {
    ...
    const revertStatus = async (applicationId: number) => {
        try {
            const response = await fetch(`http://localhost:4000/api/applicants/revert-status/${applicationId}`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json'
                }
