...
const RecruitmentPage: React.FC = () => {
    ...
    useEffect(() => {
        if (companyId) {
            axios.get(`http://localhost:4000/api/jobs/${companyId}`)
                .then((response) => {
    ...
    const handleStatusChange = (jobId: number, newStatus: string) => {
        axios
            .post(`http://localhost:4000/api/jobs/update/${jobId}`, { status: newStatus })
            .then((response) => {
    ...
    const deleteJob = () => {
        ...
        axios
            .delete(`http://localhost:4000/api/jobs/delete/${selectedJobId}`)
            .then(() => {
