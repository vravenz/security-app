...
const JobOfferResponsePage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchJobOfferDetails = async () => {
            try {
                const response = await fetch(`http://localhost:4000/api/applicants/offers/${offerId}`);
                if (response.ok) {
    ...
    const handleResponse = async (status: 'Accepted' | 'Rejected') => {
        ...
        try {
            const response = await fetch(`http://localhost:4000/api/applicants/offer-response/${offerId}`, {
                method: 'POST',
