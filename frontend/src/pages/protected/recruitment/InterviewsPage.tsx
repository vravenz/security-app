...
const InterviewPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchInterviews = async () => {
            if (companyId) {
                try {
                    const response = await fetch(`http://localhost:4000/api/applicants/interviews/${companyId}`);
                    if (response.ok) {
    ...
    const updateInterviewOutcome = async () => {
        if (selectedInterviewId) {
            const response = await fetch(`http://localhost:4000/api/applicants/interviews/outcome/${selectedInterviewId}`, {
                method: 'PUT',
            ...
            if (response.ok) {
                ...
                closeModal();
                const updatedInterviews = await fetch(`http://localhost:4000/api/applicants/interviews/${companyId}`).then(res => res.json());
    ...
    const handleDeleteInterview = async (interviewId: number) => {
        if (window.confirm('Are you sure you want to delete this interview?')) {
            const response = await fetch(`http://localhost:4000/api/applicants/interviews/${interviewId}`, {
                method: 'DELETE'
            });
            if (response.ok) {
