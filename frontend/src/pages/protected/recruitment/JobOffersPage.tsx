...
const JobOffersPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchPassedApplicants = async () => {
            if (companyId) {
                const response = await fetch(`http://localhost:4000/api/applicants/passed/${companyId}`);
                if (response.ok) {
                    const data = await response.json();
    ...
    const handleSendOffer = async (applicant: Applicant) => {
        ...
        const response = await fetch(`http://localhost:4000/api/applicants/send-offer/${applicant.application_id}`, {
            method: 'POST',
