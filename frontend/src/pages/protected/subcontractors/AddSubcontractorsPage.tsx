...
const SubcontractorSearchPage: React.FC = () => {
    ...
    const handleSearch = async () => {
        try {
            const response = await axios.get(`http://localhost:4000/api/subcontractors/search?email=${email}`);
            if (response.data) {
                setSubcontractor({
    ...
    const handleRequestEmployees = async () => {
        ...
        try {
            const response = await axios.post('http://localhost:4000/api/subcontractors/request', dataToSend);
