...
const ContractInfoPage: React.FC = () => {
        ...
        const fetchDetails = async () => {
            try {
                const [contractRes, employeeRes] = await Promise.all([
                    fetch(`http://localhost:4000/api/subcontractor-company/contract/${contractId}`),
                    fetch(`http://localhost:4000/api/subcontractor-company/employee-requests/${contractId}`)
                ]);
    ...
    const handleUpdateRequestStatus = async (requestId: number, newStatus: string) => {
        try {
            const response = await fetch(`http://localhost:4000/api/subcontractor-company/employee-request/update-status/${requestId}`, {
                method: 'PATCH',
                headers: {
    ...
    const fetchApplicationsForRequest = async (requestId: number): Promise<Application[]> => {
        try {
            const response = await fetch(`http://localhost:4000/api/subcontractor-company/applications/request/${requestId}`);
            if (response.ok) {
